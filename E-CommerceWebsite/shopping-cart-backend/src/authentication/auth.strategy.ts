...
export const configureAuthStrategy = (server: Server) => {
  ...
  server.auth.strategy("jwt", "jwt", {
    ...
    validate: (artifacts: any, request: any, h: any) => {
      // artifacts - the object the plugin passes containing decoded token data and other info.
      const payload = artifacts.decoded.payload as JWTPayload;

      // reject non-access tokens on protected APIs
      ...
      return {
        ...
        credentials: {
          // an object you create containing information about the authenticated user.
