...
export const deleteCartItemHandler = async (request: Request, h: ResponseToolkit): Promise<ResponseObject> => {
  try {
    ...
    const deletedID = await cartItemsServices.deleteCartItem(id);
    if (!deletedID) {
